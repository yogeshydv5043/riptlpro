...
function animateOnScroll() {
    const elements = document.querySelectorAll('.hero-content, .hero-image, .about-image, .about-text, .service-card, .portfolio-item, .blog-card, .contact-info, .contact-form, .stat-card, .team-card, .feature-item, .framework-card');
...
// ==================== FRAMEWORK CARDS FADE-UP ANIMATION ====================
// START: Framework Cards Scroll Animation
...
// END: Framework Cards Scroll Animation
...
// Additional animation for framework icons on hover
document.querySelectorAll('.framework-card').forEach(card => {
    card.addEventListener('mouseenter', function () {
        this.querySelector('.framework-icon').style.transform = 'scale(1.1) rotate(5deg)';
    });

    card.addEventListener('mouseleave', function () {
        this.querySelector('.framework-icon').style.transform = 'scale(1) rotate(0)';
    });
});

// ==================== PROFICIENCY BARS ANIMATION ====================
// Track if proficiency bars have been animated
let proficiencyAnimated = false;

// Function to animate proficiency bars
function animateProficiencyBars() {
    if (!proficiencyAnimated) {
        ...
        proficiencyBars.forEach(bar => {
            ...
            // Set a small timeout to ensure the reset is rendered before animating
            setTimeout(() => {
...
// Check if frameworks section is in view
function checkFrameworksSection() {
    const frameworksSection = document.getElementById('frameworks');
    if (frameworksSection) {
        const sectionPosition = frameworksSection.getBoundingClientRect();
...
window.addEventListener('scroll', () => {
    ...
    checkFrameworksSection();
});

// Also trigger on page load if section is in view
window.addEventListener('load', () => {
    animateOnScroll();
    checkFrameworksSection();
});
